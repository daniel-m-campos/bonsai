...
_CELL_DEFAULTS = {
    **{
        k: params.SCALING[k]
        for k in ("bins", "depth", "iters", "lr", "seed", "min_data_in_leaf", "lambda_l2")
    },
    "informative": 20,
...
def make_cell(defaults: dict, **over) -> dict:
    ...
    if unknown:
        raise ValueError(
            f"unknown key(s) in spec defaults: {sorted(unknown)}; legal: {sorted(_CELL_DEFAULTS)}"
        )
...
def expand(
    spec: dict, *, variants: list[str] | None = None, repeats: int | None = None
) -> list[dict]:
