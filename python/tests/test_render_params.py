...
COMMITTED = {"tree": {"max_depth": 6, "min_data_in_leaf": 20}, "boost": {"learning_rate": 0.05}}
...
def test_extraction_drift_names_every_moved_knob():
    ...
    fresh = {"tree": {"max_depth": 8, "num_leaves": 63}, "boost": {"learning_rate": 0.05}}
    assert render_params.extraction_drift(COMMITTED, fresh) == [
        "added tree.num_leaves",
        "removed tree.min_data_in_leaf",
...
def test_extract_check_refuses_a_stale_file_without_writing(monkeypatch, tmp_path, capsys):
