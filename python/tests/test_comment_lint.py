...
def test_perf_with_a_number_in_the_block_passes(tmp_path):
    ...
    path = _source(
        tmp_path, "src/other/a.cpp", "// perf: measured at 3.2 ms on the L40S\nvoid fast();\n"
    )
...
def test_perf_without_any_number_is_a_finding(tmp_path):
    ...
    path = _source(
        tmp_path, "src/other/a.cpp", "// perf: this is faster on modern hardware\nvoid slow();\n"
    )
    assert comment_lint.block_findings(path) == [
        (
            1,
            "perf: carries no number, so it is not a measurement:"
...
def test_perf_block_at_end_of_file_has_no_declaration(tmp_path):
    ...
    path = _source(tmp_path, "src/other/a.cpp", "void f();\n// perf: nothing measured here\n")
    assert comment_lint.block_findings(path) == [
        (2, "perf: carries no number, so it is not a measurement: // perf: nothing measured here"),
    ]
...
def test_sync_naming_a_construct_the_code_lacks_is_a_finding(tmp_path):
    ...
    path = _source(
        tmp_path, "src/other/a.cpp", "// sync: the atomic counter must not tear\nvoid tear();\n"
    )
    assert comment_lint.block_findings(path) == [
        (1, "sync: names no construct this file uses: // sync: the atomic counter must not tear"),
    ]
...
def test_sync_naming_nothing_is_a_finding(tmp_path):
    ...
    assert comment_lint.block_findings(path) == [
        (
            1,
            "sync: names no construct this file uses:"
            " // sync: ordering matters here for correctness",
        ),
    ]
...
def test_ffi_naming_no_boundary_construct_is_a_finding(tmp_path):
    ...
    path = _source(
        tmp_path, "src/python/a.cpp", "// ffi: the boundary must stay tidy\nvoid tidy();\n"
    )
    assert comment_lint.block_findings(path) == [
        (1, "ffi: names no boundary construct this file uses: // ffi: the boundary must stay tidy"),
    ]
...
def test_ffi_outside_src_python_is_a_finding(tmp_path):
    ...
    assert comment_lint.block_findings(path) == [
        (
            1,
            "ffi: only src/python/ may carry this tag:"
            " // ffi: the capsule owns the buffer past the call",
        ),
    ]
...
def test_untagged_finding_truncates_at_eighty_characters(tmp_path):
    ...
    assert comment_lint.block_findings(path) == [
        (1, "// this comment explains the loop at length and keeps on explaining well past ei"),
    ]
...
def test_trailing_comment_after_code_is_a_finding(tmp_path):
    ...
    path = _source(tmp_path, "src/other/a.cpp", "int x = 1;  // set the width\n")
...
def test_trailing_tagged_comment_after_code_passes(tmp_path):
    ...
    path = _source(tmp_path, "src/other/a.cpp", "int x = 64;  // perf: one cache line\n")
    assert comment_lint.block_findings(path) == []
...
def test_double_slash_inside_a_string_is_not_a_comment(tmp_path):
    ...
    path = _source(tmp_path, "src/other/a.cpp", 'const char* url = "http://example.com";\n')
    assert comment_lint.block_findings(path) == []
...
def test_a_trailing_comment_past_a_string_is_a_finding(tmp_path):
    ...
    path = _source(
        tmp_path, "src/other/a.cpp", 'const char* s = "a";  // this explains the string\n'
    )
    assert comment_lint.block_findings(path) == [
