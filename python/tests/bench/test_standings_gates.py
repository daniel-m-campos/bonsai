...
def _point_at(monkeypatch, root: pathlib.Path) -> None:
    ...
    monkeypatch.setattr(check_standings, "MODEL_HASH_SCRIPT", root / "scripts" / "model_hash.py")
...
def test_release_gate_names_the_never_refreshed_axis(monkeypatch, tmp_path):
    ...
    _fake_tree(tmp_path)
    _point_at(monkeypatch, tmp_path)
    reg = {"gpu-wide": {"file": None, "sha": None, "plane": "gpu", "refreshed_for": None}}
...
def test_quality_axis_keeps_the_whole_implementation_digest(monkeypatch, tmp_path):
    ...
    _fake_tree(tmp_path)
    _point_at(monkeypatch, tmp_path)
    assert check_standings.plane_digest(check_standings.PLANE_GPU) == check_standings.plane_digest(
...
def _ab_registry(monkeypatch, tmp_path, rows: list[dict], decisions: str = DECISIONS) -> dict:
    """A registry whose gpu-tall carries an ab file holding ``rows``."""
    results = tmp_path / "benchmarks" / "results"
    results.mkdir(parents=True)
    (results / "ab-gpu-2026-09.jsonl").write_text("".join(json.dumps(r) + "\n" for r in rows))
    ...
    return {
        "gpu-tall": {"plane": "gpu", "as_of_decision": 121, "ab": "ab-gpu-2026-09.jsonl"},
...
def test_a_moved_ab_ships_under_an_entry_tagged_with_its_axis(monkeypatch, tmp_path):
    reg = _ab_registry(
        monkeypatch, tmp_path, [_arm("anchor", 10.0), _arm("old", 10.0), _arm("new", 10.3)]
    )
    assert check_standings.check_ab(reg) == []
    assert check_standings.check_decisions(reg) == []


def test_a_citation_under_another_axis_tag_does_not_count(monkeypatch, tmp_path):
    reg = _ab_registry(
        monkeypatch,
        tmp_path,
        [_arm("anchor", 10.0), _arm("old", 10.0), _arm("new", 10.3)],
        decisions=DECISIONS.replace("Standings: gpu-tall", "Standings: cpu-tall"),
    )
    errors = check_standings.check_ab(reg)
...
def test_a_skipped_arm_carries_no_cell(monkeypatch, tmp_path):
    reg = _ab_registry(
        ...
        tmp_path,
        [
            {"arm": "anchor", "grower": "cuda_depthwise", "skipped": "no cuda"},
            _arm("old", 10.0),
            _arm("new", 10.1),
        ],
        decisions="## 1. Nothing\n",
    ...
    assert check_standings.check_ab(reg) == []
    rows = check_standings.ab_rows(tmp_path / "benchmarks" / "results" / "ab-gpu-2026-09.jsonl")
...
def test_tagged_entries_carry_every_axis_of_a_tag_and_skip_untagged_ones():
    ...
    assert set(check_standings.tagged_bodies(text)) == {"gpu-tall", "cpu-tall", "cpu-wide"}


def test_check_decisions_still_holds_a_tag_newer_than_the_registry(monkeypatch, tmp_path):
...
def _quality_row(variant: str, dataset: str, seed: int, value: float, status: str = "ok") -> dict:
    return {
        "suite": 297,
        "dataset": dataset,
        "variant": variant,
...
def _drift_registry(monkeypatch, tmp_path, cpu: list[dict], gpu: list[dict]) -> dict:
    """A registry whose quality axes hold ``cpu`` and ``gpu`` rows."""
    results = tmp_path / "benchmarks" / "results"
    results.mkdir(parents=True)
    for name, rows in (
        ("quality-grinsztajn-2026-09.jsonl", cpu),
        ("quality-grinsztajn-gpu-2026-09.jsonl", gpu),
    ):
        (results / name).write_text("".join(json.dumps(r) + "\n" for r in rows))
    monkeypatch.setattr(check_standings, "RESULTS", results)
    return {
        "quality-grinsztajn": {"file": "quality-grinsztajn-2026-09.jsonl"},
        "quality-grinsztajn-gpu": {"file": "quality-grinsztajn-gpu-2026-09.jsonl", "plane": "gpu"},
    }
...
CPU_ROWS = [
    _quality_row("bonsai_dw", "wine", 0, 0.500),
...
def test_a_device_plane_inside_the_host_spread_holds(monkeypatch, tmp_path):
    ...
    drifts = {
        d.grower: d
        for d in check_standings.quality_drift(
...
def test_an_unmeasured_device_axis_is_not_gated(monkeypatch, tmp_path):
    ...
    assert check_standings.drift_pairs(reg) == [("quality-grinsztajn-gpu", "quality-grinsztajn")]
