...
def _run_pct(periods: int, throttled: int) -> subprocess.CompletedProcess:
    return subprocess.run(
        ["awk", "-v", f"p={periods}", "-v", f"t={throttled}", _throttle_pct_program()],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
    )
...
@pytest.mark.parametrize(
    "periods,throttled,expected",
    [(169, 20, "11.8"), (558, 0, "0.0"), (100, 100, "100.0"), (0, 0, "0.0")],
)
...
def test_the_gate_fails_closed_when_the_probe_reads_nothing():
    ...
    assert guard, (
        "the pct guard is gone: an empty or non-numeric percentage "
...
def test_the_limit_is_compared_as_a_number():
    ...
    over = subprocess.run(
        ["awk", "-v", "pct=11.8", "-v", "max=5", "BEGIN {exit !(pct + 0 > max + 0)}"]
    )
    under = subprocess.run(
        ["awk", "-v", "pct=0.9", "-v", "max=5", "BEGIN {exit !(pct + 0 > max + 0)}"]
    )
...
def _checkout_sha_source() -> str:
    ...
    found = re.search(r"^checkout_sha\(\) \{.*?^\}$", POD_SCRIPT.read_text(), re.S | re.M)
...
def _fake_origin(root: pathlib.Path) -> dict:
    ...
    work = root / "work"
    subprocess.run(
        ["git", "clone", "-q", "--single-branch", "--branch", "main", origin.as_uri(), str(work)],
        check=True,
    )
    return {"work": work, "side": side, "clone_head": _git(work, "rev-parse", "HEAD")}


def _run_checkout(work: pathlib.Path, sha: str) -> subprocess.CompletedProcess:
    body = _checkout_sha_source()
    return subprocess.run(
        ["bash", "-c", f'set -euo pipefail\n{body}\ncheckout_sha "$1"', "_", sha],
        cwd=work,
        capture_output=True,
        text=True,
    )
...
def test_parity_is_taken_only_by_the_session_that_anchors_it(plane, axes, taken):
    ...
    The gpu-tallest case is why the match is comma-anchored."""
        ...
        [
            "bash",
            "-c",
            f"PLANE={plane}; AXES={axes}; PARITY_AXIS=gpu-tall\n"
...
def test_the_arms_are_the_wheels_the_driver_named_plus_head(anchor, prev, arms):
    ...
    have nothing to do."""
        ...
        [
            "bash",
            "-c",
            f"set -eu; ANCHOR_VERSION='{anchor}'; PREV_VERSION='{prev}'\n"
            f"{_arm_list_source()}\n"
            'echo "${AB_ARMS[*]}"',
        ],
        capture_output=True,
...
def _cpu_ab_guards() -> dict[str, str]:
    ...
    found = [
        ln.strip().removeprefix("if ").removesuffix("; then")
        for ln in POD_SCRIPT.read_text().splitlines()
...
def test_the_cpu_ab_rides_the_tall_axis_only(axis, taken):
    ...
    assert _takes(_cpu_ab_guards()["axis"], f"axis={axis}; CPU_AB_AXIS=cpu-tall") == taken
...
def test_the_cpu_ab_runs_alone_on_a_cpu_plane_that_did_not_fit_the_tall_axis(plane, axes, taken):
    ...
    assert (
        _takes(_cpu_ab_guards()["alone"], f"PLANE={plane}; AXES={axes}; CPU_AB_AXIS=cpu-tall")
        == taken
    )
...
def test_the_driver_registers_the_cpu_ab_under_the_axis_the_pod_fits_it_at():
    ...
    assert standings_refresh.AB_AXES[standings_refresh.PLANE_GPU] == (standings_refresh.PARITY_AXIS)
