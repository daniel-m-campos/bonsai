# Ordered-boosting admission probe, stage 0: is CatBoost's small-data edge the ordered mechanism? (2026-07)

Decision 80 left an open thread: after ablating CatBoost's categorical machinery, a residual small-data lead over bonsai persisted on pure-numeric data (the cat-probe control was outlier-best for CatBoost on 5 of 6 pure-numeric regression sets), and that residual was pinned on "ordered boosting / oblivious regularization / defaults" without decomposing which. Ordered boosting is the marquee CatBoost mechanism: per-row gradients are computed by models that never saw the row, removing the prediction shift (target leakage) of ordinary boosting. This probe is stage 0 of the ordered-boosting campaign: price that mechanism at zero bonsai-core cost with CatBoost's own reference-library toggle (feature-admission step 1), separate it from CatBoost's strong tuned defaults, and test whether an honest-gradient prototype reaches it outside a full permutation engine.
