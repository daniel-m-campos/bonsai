...
What remained after the accuracy fix was a speed gap — and most of it was a second bonsai issue, not catboost's kernel. Binning ran an O(n) reservoir pass *per feature* (mapper-fit ~5.7s of the 16M GPU fit, serial host time catboost doesn't pay); one shared row sample for the whole matrix cut it to 0.77s, quality-neutral (decision 64). Same pod, 16M, 100 iters: `cuda_oblivious` **19.65s vs catboost 18.85s** at matched accuracy — a **~4% residual**, down from ~19%. What's left is genuine per-round symmetric-tree kernel efficiency, a bounded target for a later round, not a disadvantage that requires changing what bonsai is.
