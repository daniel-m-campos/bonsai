...
Residual, filed as **#63**: house_sales' remaining +8% is continuous-column cut *placement* at the shared 255-bin budget (bonsai at 1023 bins beats every ref at their 255) — in tension with decision 55's synthetic-cell verdict, which is exactly why real datasets were worth the campaign.
...
|---|---|--:|--:|--:|--:|--:|--:|
| house_sales | rmse | 131,841 | 131,656 | 134,800 | **121,829** | 122,213 | 124,095 |
| superconduct | rmse | **10.3519** | 10.7470 | 11.8697 | 10.7637 | 10.7227 | 12.1154 |
| elevators | rmse | **0.0023** | 0.0023 | 0.0025 | 0.0024 | 0.0023 | 0.0026 |
| wine_quality | rmse | **0.6335** | 0.6386 | 0.6464 | 0.6385 | 0.6386 | 0.6505 |
| magic_telescope | auc | **0.9360** | 0.9346 | 0.9314 | 0.9330 | 0.9352 | 0.9335 |
| phoneme | auc | **0.9435** | 0.9413 | 0.9294 | 0.9283 | 0.9379 | 0.9396 |
| electricity | auc | **0.9458** | 0.9414 | 0.9155 | 0.9353 | 0.9441 | 0.9178 |
| letter | acc | **0.9613** | 0.9603 | 0.9557 | 0.9240 | 0.9610 | 0.9223 |
| satimage | acc | **0.9191** | 0.9176 | 0.9137 | 0.9012 | **0.9191** | 0.9059 |

**bonsai-depthwise is the best-scoring library on 8 of 9 datasets** (sole exception: house_sales, #63). Leafwise sits within noise of depthwise throughout; oblivious carries its structural quality trade but now beats catboost — the other symmetric-tree library — nearly everywhere. Sampler variants (bernoulli, goss) tracked their all_rows baselines within expected sampling noise across the full pre-fix matrix (`q_*` files). The bike_sharing (Poisson) cells run once PR #55 merges — the objective is newer than this campaign's build.
