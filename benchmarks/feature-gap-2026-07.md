...
> **Frozen record.** The parity tracker as it stood when the last of its rows closed, kept because decisions 34, 35, and 38 cite its A/B tables as their evidence. It is no longer a live tracker: the datasets it measures on (California Housing, Year Prediction MSD) are retired, and several result files it names have been deleted. Current feature status is [decisions.md](../docs/decisions.md); current numbers are [the results ledger](../docs/results/results.md).
