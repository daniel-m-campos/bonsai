...
Every step passed; the ladder never hit the card. Wall time is the whole `train()` call: chunked host-to-device ingest, device binning, mapper fit, and all 60 boosting rounds. Device memory grows at ~13.9 GiB per 100M rows (1 byte per cell of binned matrix plus ~4.2 bytes per row of resident fit state), which extrapolates the true ceiling to roughly 570M rows x 100 on this card.
...
Synthetic data (the bench Friedman-1 recipe, chunked with a pilot-chunk noise scale), one pod, one run per step; wall times carry the usual ~25% fleet spread and the r2 column is a 1M-row train-sample proxy, not a held-out test. The ~570M extrapolated ceiling is arithmetic beyond the last measured step, not a measurement. Ingest dominates the wall time at this scale (the 60-round boosting portion is a minority of the 512.6s); workloads reusing a `bonsai.Dataset` across fits pay it once.
