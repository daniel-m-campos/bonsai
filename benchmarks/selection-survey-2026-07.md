...
4. **shap_train and shap_val are the same ranking here** (identical cells except one rung): with 11k clean rows, held-out attribution does not change the order. rfe matches gain (recursion has nothing to repair at these k). perm matches gain at 4.5x the wall.
