...
| wrapper | rfe_gain | backward recursive elimination down the ladder budgets, gain recomputed at each step; drop order = ranking | one fit per step |
...
| wrapper | rfe_val | sequential backward elimination scored by validation error: drop the feature whose removal, evaluated by a cheap refit in the context of the current set, costs the least val error; reversed drop order = ranking | ~3,320 cheap fits |
...
4. **shap_train and shap_val are the same ranking here** (identical cells except one budget): with 11k clean rows, held-out attribution does not change the order. Suspiciously equal, so it was checked: the two importance vectors are genuinely different (different input matrices, max element gap 0.13, both contribution matrices pass the sum-to-prediction identity) and their rankings first diverge at position 21, but the top-k SETS coincide at every budget except k=32 (symmetric difference 2), and an identical set refits deterministically to the identical rmse. The one differing cell is the k=32 pair. rfe matches gain (recursion has nothing to repair at these k). perm matches gain at 4.5x the wall.
5. **The set-evaluating wrappers beat every ranking at k <= 12, by more than the noise floor.** forward leads (10.168 vs 10.404 at k=12, a 0.24 gap against the 0.197 floor; 10.428 at k=8; 11.416 vs 12.553 and worse at k=4), and rfe_val, its backward mirror scored by validation error, lands between forward and the rankings (10.283, 10.480, 11.625 at k=12, 8, 4), within one floor of forward at k=8 and 12. These two are the only methods that evaluate feature SETS rather than individuals, so they are the only ones that pick complements instead of redundant twins; the overlap diagnostic says it in one number each (Jaccard vs gain at k=16: forward 0.28, rfe_val 0.455, every ranking 0.78 or higher). The costs: forward pays 239 s, roughly 50x the gain ranking; rfe_val pays 1090 s, 4.6x forward, because backward candidate fits carry nearly all features early where forward's carry few. rfe_val also trails the one-fit rankings at LOOSE budgets (out of band against the baseline at k=32 where gain is in band): 3,320 val queries let greedy drops chase validation noise the one-fit rankings never touch, the val-overfitting caveat made visible.
6. **Error-awareness is fuel, set-awareness is the engine.** Per-feature error scoring (perm_val) bought nothing over the error-blind rankings; removal-in-context error scoring (rfe_val) bought about half of forward's tight-budget advantage; full set construction (forward) remains the best and the cheaper of the wrapper pair. The validation target powers the wrappers, but what separates them from the rankings is evaluating candidates in the context of what is already kept.
...
3. An RFE ranking-reconstruction bug in the first run (dropped batches concatenated in the wrong order) was caught by the structural identity check (rfe top-512 must equal gain top-512 on QSAR because the first step's survivors are exactly gain's top-512, and the k=512 cells must match; they read 0.945 vs 0.895) and fixed; both rfe arms were rerun with the corrected reconstruction. The committed jsonl contains only corrected rows, and the identity now holds exactly (0.89518 both).
4. rfe_val was added after the first release of this survey, prompted by the question of whether error-in-the-loop selection beats the error-blind SHAP rankings. The pre-registered prediction, on record before the run: it lands between rfe_gain and forward, closer to forward at tight k. It did. Its candidate scoring uses the same validation slice as every arm's early stopping (identical information budget across methods); the reported numbers are holdout like every arm.
5. This survey supersedes the guide14 budget-curve addendum in `benchmarks/feature-selection-probe-2026-07.md`; the gain arm here reproduces those rows digit-for-digit at every shared k.
