...
**catboost owns the frontier at every accuracy you would actually ship.** Above ~0.875 r², catboost is unbeaten: catboost@150 (28.35s, 0.8892) is both faster *and* more accurate than bonsai `cuda_depthwise`@100 (30.16s, 0.8776). Measured against the *structural* match — bonsai's symmetric `cuda_oblivious` — catboost's lead is two independent gaps at once, both visible at a fixed 100 iterations: it runs ~15% faster per round (0.24s vs 0.28s, its tuned symmetric-tree kernel) **and** lands +0.011 test r² per round (0.8751 vs 0.8638, its ordered boosting correcting the prediction-shift bias that ordinary gradients carry). bonsai's depthwise grower claws back the accuracy-per-round gap with more expressive trees (0.8776 vs 0.8751 at 100), but pays for it in speed, so the frontier still goes to catboost. bonsai holds the frontier only in the sub-24s / sub-0.85 corner — the fast-and-rough regime.
