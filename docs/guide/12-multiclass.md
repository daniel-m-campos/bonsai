...
- **What composes for free, and what doesn't**: bagging, GOSS, every grower (including the CUDA ones — the engine sees $K$ ordinary trees per round), feature importance, warm start, and the model format all work unchanged, because they never knew about classes. The seams that *do* know the output shape — SHAP contributions — are per-class by nature and currently throw for multiclass; that asymmetry is the honest cost of the separate-booster design.
