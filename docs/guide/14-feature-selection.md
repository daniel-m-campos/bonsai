...
2. **Loose budgets are free and method-indifferent.** Down to $k{=}32$ (a 2.5x reduction) every model-based ranking ties the baseline (the gap is smaller than the noise floor), and even zero-cost `corr` does. Paying 33s for recursive elimination to reach a place a free filter reaches is money burned.
