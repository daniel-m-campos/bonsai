...
- **The inflation grows with $p$ and shrinks with $n$.** More features means more lottery tickets per node; more rows means each candidate's null gain concentrates toward zero. That is why wide-short is the danger regime, and why the same junk columns that wreck a 550-row fit are ignored at 5,000 rows.
...
- **Noise removal**: drop columns carrying no signal. Rankings can do this, because junk sits at the bottom on average.
...
**Validation scoring**: `perm_val`, permutation importance, shuffles one column at a time on the validation set and charges the feature the resulting error increase. No refits, only predicts, but one pass per feature per repeat.
...
The judge is the same for every method: take the method's ranking, refit at matched knobs on its top-$k$ for each rung of a budget ladder ($k$ = 64 down to 4), and read rmse on an untouched holdout. Selection cost is wall-clock for producing the ranking. All deltas are judged against the chance band (2% relative, 0.197 rmse here). Full protocol, tables and raw rows: [`benchmarks/selection-survey-2026-07.md`](../../benchmarks/selection-survey-2026-07.md) and [`scripts/probe_selection_survey.py`](../../scripts/probe_selection_survey.py).
...
2. **Loose budgets are free and method-indifferent.** Down to $k{=}32$ (a 2.5x reduction) the model-based rankings are all in band, and even zero-cost `corr` is. Paying 33s for recursive elimination to reach a place a free filter reaches is money burned.
3. **Tight budgets separate the families.** At $k{=}16$ the embedded rankings hold ~10.2 while `mutual_info` is at 11.0, four band-widths worse: filters flood their top ranks with copies of one strong cluster (their top-5 here contains three variants of the same ThermalConductivity family). `split` is the worst embedded ranking at every tight rung, chapter 8's cardinality bias made visible.
4. **The one-shot model rankings are interchangeable, and so are their refinements.** `gain`, `shap_val`, `perm_val`, `rfe_gain` track each other inside the band all the way down; recursion and validation-scoring have nothing to repair here because (per the bootstrap diagnostic) the gain ranking is already stable, with a core of 8 features chosen in 10 of 10 resamples.
5. **Forward selection wins every rung at $k \le 12$, beyond the band.** 10.168 vs 10.404 at $k{=}12$; 11.416 vs 12.553-13.921 at $k{=}4$, more than a full band better than everything else. This is the correlation section's prediction landing: forward evaluates *sets*, so it picks complements where every ranking picks redundant twins. The overlap diagnostic makes it concrete: forward's top-16 shares only Jaccard 0.28 with gain's, and its first five picks span five *different* property families where `corr`'s span one. It paid 239s, about 50x the gain ranking, and at tight budgets that money bought real accuracy.

**The wide-short footnote (QSAR-TID-11, 1,024 features).** At 81 features the rankings were interchangeable; at 1,024 they finally separate, and in the direction the inflation math predicts: `shap_val` becomes the best ranking (in band at $k{=}256$, a free 4x reduction; ahead of raw `gain` by 1.4 to 2.9 band-widths at $k \le 128$), because attribution on rows the fit never memorized discounts the lottery winners that wide-short data mass-produces. `perm_val` matches it only at the tightest rung and pays 164s against `shap_val`'s 3.7s. Forward is absent by honest arithmetic: at $p{=}1024$ its candidate count is ~24,000 fits.
...
Both motivations in one miniature: junk columns at small $n$, then the budget curve.
...
- **Importance is also a leakage detector.** An ID-like or timestamp column at the top of the gain ranking is not a feature to keep; it is a bug report about your data. The wide-short lottery makes this *more* likely on exactly the datasets where the ranking looks most convincing.
...
- **Selection is still worth it when accuracy is not the point.** A third of the features means a third of the pipeline to compute, monitor, and explain, and every dataset here hands you a 2.5-4x reduction inside the chance band for free. The budget case is the reason this chapter exists.
