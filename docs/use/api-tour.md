...
Prediction stays a host call, and a `Dataset` serves it too: every X-taking `Model` method (`predict`, `predict_proba`, `staged_predict`, `predict_leaf`, `pred_contribs`) takes a `Dataset` wherever it takes an array, reading the host matrix a host-built `Dataset` retained, so `model.pred_contribs(ds_test)` works without keeping `X` around. A `Dataset` built from device-resident input kept no host rows, so those methods raise with the remedy instead; on-device prediction is a separate concern.
