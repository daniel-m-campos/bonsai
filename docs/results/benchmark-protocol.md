...
|---|---|---|---|---|---|---|
| grinsztajn | quality | 55 external tasks (quality-external) | r² / AUC | `results/grinsztajn-2026-08.jsonl` | `python -m bonsai.bench.grinsztajn` | 68 |
...
Perf-division data is a generalized form of Friedman #1 (Friedman, "Multivariate Adaptive Regression Splines", 1991), the standard synthetic regression for tree methods: the classic five-feature target 10 sin(pi x0 x1) + 20 (x2 - 0.5)^2 + 10 x3 + 5 x4 repeated over the informative features in blocks of five with 0.6^b decaying block weights, uniform [0, 1) float32 features, and Gaussian noise with sigma = y.std() / 3, which places the best-achievable test r² near 0.9. Synthetic is kept for perf deliberately: rows, columns, and bins become free experimental axes (decision 46), and generation is deterministic in (seed, rows, cols, n_test) via SeedSequence and in nothing else, so bin and thread sweeps reuse byte-identical data. The implementation and full provenance: `bonsai.bench.synth`. A deliberately frozen linear variant lives in `scripts/model_hash.py`; its output feeds the cross-architecture byte-identity CI gate and must never be edited.

The rows are drawn in a fixed number of row blocks off spawned streams, which is what lets generation use the whole container instead of one core (decision 112). The block count is part of the data and the worker count is not: the same cell produces the same bytes on a 27-core pod and a 128-core one. Each row records the `data_recipe` it was generated under, because the recipes are not byte-comparable with each other and a results directory holding both must read as mixed rather than as a comparison. **Rows carrying `data_recipe` 1, or no such field, were generated by the single-stream recipe and cannot be compared against recipe-2 rows.** The recipe is also part of the `BONSAI_BENCH_DATA_CACHE` key, so a cache written under one never satisfies a run measuring under the other.
...
A cell may add a fourth phase, contribs_s: one TreeSHAP call over the whole test matrix, timed after predict_s. It exists only on cells that opt in (the gpu-shap suite), so it never appears on a legacy row. Every arm also reports contribs_additivity, the worst per-row relative gap between its own SHAP values' row sum and that arm's own raw prediction. A library's SHAP call runs wherever that library puts it, CPU or GPU, and the row records which.
...
The release gate has one mechanical exception, scoped to quality axes (a registry key starting with `quality`). A quality axis's slow clock, "refresh on engine quality changes or reference-library majors," used to be enforced by memory alone. It is now a check: the axis's committed `hash_set` (a digest over `scripts/model_hash.py` and the whole `src/`/`include/` implementation it drives) must still equal a fresh digest, and none of the axis's `refs` reference libraries may have bumped a major version since. Bit-identical CPU model-hash bytes cannot move quality standings by construction (decision 40's contract: CPU-only builds stay bit-identical), so an unmoved guard proves the axis current without spending pod time on it. Perf axes carry no such proof, since their numbers are wall-clock rather than bytes, and still require an exact `refreshed_for` match. `scripts/update_standings.py` stamps `hash_set` and `refs` whenever it supersedes a quality axis; the reference-library versions it records are the ones the results rows carry in `host.libs` (every sweep stamps the imported library's version on each row, so the refs name the sweep's own environment, and a file whose rows disagree on a version is refused), completed for a library no row exercised from the installed environment when available and otherwise from the `benchmarks/reference_versions.json` ledger, since the bench extras are unpinned in `pyproject.toml` and there is no lockfile to read statically.

The device plane's quality is held against the host's by a third gate. `quality-grinsztajn-gpu` sweeps the same suite, seeds, and campaign knobs with bonsai's three CUDA growers alone, so every device row has one CPU row of the same suite, dataset, seed, and growing strategy. The two fits are not bit-identical (fixed-point cells, the device's own tie order), so the metrics differ in the last places, and by how much is a property of the task: the scores are read on held-out rows of tasks as small as 2043 training rows, where a different split at one near-tie cut moves the score by more than 1e-3, which is also what moves it between the host's own three seeds (decision 128 measured the first sweep against a fixed 1e-3 band and found 33 pairs past it, every one inside its task's seed spread, which ran 1.15 to 145 times the gap). `scripts/check_standings.py` therefore reads each pair against its task: the gap holds inside the host's seed-to-seed spread of that suite, dataset, and strategy, plus `QUALITY_DRIFT_FLOOR` (1e-4, the host-vs-device prediction tolerance, so a single-seed task still has an allowance), at both claim time and release time, and the quality page tables the mean, the worst pair, and the spread it sits against. A gap past its task's spread is a device-plane bug until a decision says otherwise.

A reference library that does not take the campaign knobs on its device is met at the knobs it does take, on its own axis. LightGBM's CUDA learner caps a tree by its leaf count alone, so `quality-grinsztajn-leaf-capped-gpu` sweeps `python -m bonsai.bench.grinsztajn --device cuda --regime leaf-capped`: the campaign's 63 leaves under a depth cap of 62, the deepest a 63-leaf tree can reach, on the two learners a leaf count alone can cap (bonsai's leafwise grower and LightGBM), every other knob the campaign's. The leaf-capped regime is its own axis and its own table, ranked by the same rule, never merged into the campaign standings, and never drift-paired: no host axis runs the regime, and the regime's question is head to head on one device, not device against host (decision 130).
...
The refresh is one rented pod session, driven locally by `scripts/standings_refresh.py` (decision 96; section 10 of the RunPod runbook): a same-pod A/B of the previous release wheel and one fixed anchor wheel (1.15.0) against HEAD on anchor cells, on both planes, detects whether perf moved, then the standings specs re-measure the axes and the supersession lands as one reviewed PR. The statistic is the min over interleaved reps, since noise on a fixed workload only adds time. HEAD is read against the previous release inside a 5% band and against the anchor inside a 2% band: the anchor makes the comparison cumulative, so a loss that hides inside the release band every release (1.5% a release compounds to 35% over twenty) shows against the anchor by the second one. A cell outside either band is a moved verdict. The A/B rows ship with the standings they were measured beside: the refresh commits each plane's file as `benchmarks/results/ab-<plane>-<date>.jsonl`, registered as the plane's tall axis's `ab` entry, so it supersedes with that axis and renders as the release drift section of the perf page. `make docs-check` refuses a moved verdict until a `Standings:` decision entry tagged with the axis cites the file and says why the move ships, or the axis is re-measured.
...
Plane gating is mechanical, so a digest sometimes reports that something which could matter moved where the tree says it could not have. A carry-forward stamp records that argument instead of buying a re-measurement: the axis keeps its results file, measured sha, host, date, and `refreshed_for`, and only its digest advances, so the published caption still shows the real vintage and the release gate still prints the stamp as a stamp. It is legitimate only under two conditions together, that the changed sources are unable to reach the axis by an argument you can state in one sentence, and that the equivalence was verified by a check you ran and recorded, and never because a refresh is merely expensive: cost is not evidence.

The stamp is spent when the plane digest next moves, because the argument was about one change and not about the axis. `scripts/update_standings.py --restamp-verified` implements the rest: the two evidence kinds (byte identity, or a bounded delta against the plane's own run-to-run noise), the fields each demands, and every refusal. A byte-identity block names the grower whose hash it carries: a host grower's hash is identity on every machine, and a device grower's hash is identity on one device and one build (decision 124), so a `cuda_*` block also names the host both hashes were measured on, and the tolerance kind remains for a plane that cannot offer identity.
