...
The entry records the argument as structured fields, in a `carried_forward` block: `measured_at` (the sha the data still comes from), `stamped_at` (the commit whose digest was carried onto it), `reason` (why the change cannot reach the axis), and `evidence` (the proof, which has a kind). The tool refuses without all of them, refuses on an axis that is already current, and refuses when the staleness comes from a reference-library major bump, which no argument about our own sources can answer.
