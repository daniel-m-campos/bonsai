...
The entry records the argument as structured fields, in a `carried_forward` block: `measured_at` (the sha the data still comes from), `stamped_at` (the commit whose digest was carried onto it), `reason` (why the change cannot reach the axis), and `evidence` (the proof, which has a kind). You supply the reason and the evidence; the tool derives `measured_at` from the axis's registered sha and `stamped_at` from HEAD, so neither can be typed in wrong. It refuses without a reason, refuses without the fields the evidence kind demands, refuses on an axis that is already current, and refuses when the staleness comes from a reference-library major bump, which no argument about our own sources can answer.

The evidence has a kind because the two planes can prove different things. `model-hash` is byte identity: the model hash at each commit, which must be equal, and any difference at all refuses. The host plane can offer that, since its model bytes are a pure function of input, configuration, and resolved thread count. The device plane cannot offer it at all. Device histogram cells are accumulated by thousands of threads under atomics, so the order the adds land in is whatever the scheduler produced, and the same fit at the same commit writes different model bytes on every run: a verification session measured four repetitions of one `cuda_depthwise` fit at a single commit and read four different model hashes. Byte identity does not exist on that plane, so a carry-forward that demanded it could not express the evidence the GPU axes actually have.

What the device plane can offer is a comparison against the noise it already carries. `tolerance` records the metric (the max absolute prediction delta between two fits), the within-commit noise floor measured at each of the two commits, the cross-commit delta, and the cell, configuration, and host the comparison ran on. It holds when the cross-commit delta stays within a fixed factor of the larger of the two floors, 2.0, recorded in the block alongside them, on the reading that a difference that small is not distinguishable from the spread the plane produces with no code change at all. That factor is a constant rather than a per-call flag, so two carry-forward records are read against the same bar and stay comparable. It refuses when the delta outruns that factor, when either floor is missing, since a delta with no scale to be read against is a number and not evidence, and when the larger of the two floors is zero, since a plane that reproduces its bytes should be proving it with `model-hash`.

The distinction is permanent by construction. The measured sha and results file never move, so the published caption still shows the real vintage; the release gate prints `carried-forward stamp` rather than `hash-unchanged skip`; and `--stale` prints a note naming the axis, the commit the stamp was carried to, the evidence kind, the sha the data is still measured at, and the reason. A carry-forward is also spent: the next time the plane digest moves, the axis is stale again like any other, because the argument was about one specific change and not about the axis.
