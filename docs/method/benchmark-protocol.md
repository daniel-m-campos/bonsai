...
The stamp is spent when the plane digest next moves, because the argument was about one change and not about the axis. `scripts/update_standings.py --restamp-verified` implements the rest: the two evidence kinds (byte identity on the host plane, a bounded delta against the plane's own run-to-run noise on the device plane; a device fit is byte-identical to itself on one device since decision 124, and teaching the restamp machinery to accept that evidence on the device plane is a follow-up), the fields each demands, and every refusal.
