...
**Consequence.** Densification mints a perfect tree, so a depth-d levelwise tree packs 2^d paths whatever its live coverage, and the nodes no training row visited pack verbatim carrying a zero cover fraction. The host walker skips those branches where the device closed form multiplies them out, which is arithmetic worth testing rather than assuming, so the levelwise SHAP parity fixtures assert their zero-cover node count is non-zero: a fixture that quietly stopped producing dead slots would pass while testing nothing. Correctness is device-verified on the Orin. Throughput is not: whether the device path beats its own host walk for a levelwise model, given that 2^d, is unmeasured and waits for a pod session, so this entry claims capability and no speed.
