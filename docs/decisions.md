...
## 67. Per-feature bin budgets: declined by measurement; the emulation is the feature (declined)
...
**Consequence.** The probe doubles as the user-facing answer: domain-mandated bins (regulatory buckets, known discontinuities) work today, bit-exactly, by pre-discretizing features to bin ids and feeding stock bonsai (the `ManualBinner` recipe in the probe script); a >256-bin feature flips the dataset to u16 storage, documented cost. The default also gained evidence: uniform `max_bin = 255` is robust in a way per-feature schemes are not.
...
**Reopen if:** a memory-constrained regime forces the global budget down (max_bin ≤ 64, where saturation no longer holds and allocation could matter), or a real workload arrives where domain-mandated bins measurably beat quantile bins.
