...
## 52. Device residency: gradients first, binning second (proposed)

**Decision (proposed).** The 16M×100 GPU gap to xgboost (38.0 vs 27.9s) is dominated not by binning but by `begin_tree` re-uploading grad/hess every tree — 12.8GB of H2D per fit, because objectives compute against host scores. Phase A makes scores/grad/hess device-resident: a fused post-tree kernel applies the score update from the device leaf assignments and computes next-iteration gradients in place (device twins for the pointwise MSE/logloss objectives under the existing dispatch; DART and leaf-renewal configs fall back to host, documented), with one D2H of scores at fit end for `route_unsampled`. Projected 38 → ~31–32s. Phase B (device binning) is demoted to the mapper-fit + ingest-bin tail (~8.7s) and sequenced after A — its first-draft premise misattributed the upload cost, and raw-float uploads (6.4GB) to save a 1.6GB bins upload are a regression while host bins remain mandatory. Full design: `docs/architecture/13-device-residency.md`, which supersedes its own device-binning-first draft — the killed draft's reasoning is retained there as the lead rejected alternative.

**Rejected.** Device binning first (misattributed target); fp16 gh compression (halves a line phase A deletes, precision risk); whole-booster device port (control flow is cheap on host; only the O(n_rows)-per-tree data plane pays for residency).
