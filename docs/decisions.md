...
## 87. An expectile objective: declined, the sample_weight hook already serves it (adopted)

**Decision.** xgboost 3.3 (2026-07-21) shipped reg:expectileerror, the smooth asymmetric-cost sibling of the pinball loss (real hessian, non-crossing curves). Priced at zero core cost per the admission method (scripts/probe_expectile.py, benchmarks/expectile-tradeoff-2026-07.md; the reference toggle plus a custom-objective math check, campaign-matched knobs, alpha in {0.75, 0.9, 0.95}). The workload is real: the true expectile fit beats MSE plus a validation-tuned constant shift by 9-57% holdout expectile loss on heteroscedastic synthetic and 2-4% on california housing, growing with alpha, because a constant shift fixes the level but cannot track a noise scale that varies with x. But bonsai already reaches it at zero core lines: a plain MSE fit followed by two sample_weight refits, weights alpha or 1 - alpha by the previous fit's residual sign, ties the native xgboost objective on every measured cell (ratios 0.99-1.02) at three fits of wall clock. The shipped quantile objective plus the same shift treatment is a partial substitute only: it ties on the symmetric synthetic and trails by 10-20% on the skewed real target, where the two functionals genuinely diverge. Declined: a native ExpectileObjective (roughly 100 lines mirroring QuantileObjective across objective.hpp/cpp, the registry, a config knob, tests, docs) would buy a 3x wall saving and one-command UX on a workload the shipped surface serves.

**Consequence.** The IRLS recipe is the documented answer and lives in the evidence doc; the objective family stays at mse, logloss, softmax, mae, huber, quantile, poisson. The custom-objective math check (grad = 2w(pred - y), hess = 2w, base score seeded with the training expectile) is recorded so an adopted objective would start from validated gradients.

**Rejected.** Adopting for UX alone (the admission bar is capability, not convenience); reading the quantile-plus-shift tie on synthetic as a general substitute (it broke by 10-20% the moment the target was skewed); scoring arms on the calibration gap instead of expectile loss (any arm composes the free validation shift for exact calibration).

**Reopener.** A user workload where the three-fit wall cost binds (large data, many alphas per model) or where IRLS-2 measurably trails the exact objective beyond the chance band; the probe's validated gradients and the QuantileObjective template make the build a leaf if that day comes.
