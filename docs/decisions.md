...
## 52. Device binning, phase 1: bin on the GPU, stream raw floats in chunks (proposed)

**Decision (proposed).** The CUDA engine bins on device: per-feature cut arrays upload once, raw float features stream through pinned staging in feature-group chunks (H2D of chunk k+1 overlapping the bin kernel of chunk k), and a branchless per-row binary search writes u8/u16 device columns bit-identical to `BinMapper::transform`. Replaces host binning (4.9s) + binned-column upload (6.0s) at 16M×100 with a ~2.5–3s raw stream — projected `bonsai_cuda` 38 → ~30s vs xgboost-GPU's 27.9s, with quantile fitting kept on host until phase 2 can prove cut-identical device fitting. Full design: `docs/architecture/13-device-binning.md`. This is the last big lever the scaling study documented (decision 46's deferred Round 5; catboost's rows exponent 0.48 vs our 0.91 is the ceiling it chases).

**Rejected.** Whole-matrix residency (busts VRAM clamps for no speed); on-the-fly binning inside the histogram kernel (re-searches per node-visit instead of once per cell); phase-1 device quantile fit (couples a models-change risk to a pure-perf change).
