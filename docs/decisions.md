...
**Consequence.** End-to-end, this removes ~5.7s of serial host time from a 16M GPU fit, taking `cuda_oblivious` from ~29.2s toward catboost's ~23.8s — closing most of the residual §63 flagged — and speeds every large CPU fit by the same ~8s. It is the standard lightgbm/xgboost approach (sample rows once); bonsai had simply not adopted it.
