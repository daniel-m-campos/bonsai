...
The levelwise (oblivious) grower rejects monotone and interaction constraints at construction rather than silently ignoring them, on both CPU and CUDA engines.

- code: `src/grower_impl.hpp` : `monotone_constraints are not supported`
- elaboration: decision 35; issue #431 records the isotonic-projection path to lifting the monotone half.
...
- code: `src/step/level.hpp` : `sibling derives by subtraction`
...
- code: `src/shap_paths.cpp` : `tree carries no per-node covers`
- elaboration: the gain-storage entry in decisions.md.

### dart-excludes-early-stopping

DART and early stopping are incompatible by construction and the combination throws: DART rescales earlier trees each round, which invalidates incrementally accumulated validation scores.

- code: `src/cli/pipeline.cpp` : `early_stopping_rounds cannot be combined`
- elaboration: the DART entry in decisions.md.
...
- code: `include/bonsai/shap.hpp` : `efficiency property holds exactly`
...
- code: `src/python/module.cpp` : `Rebuild it from X and y`
