...
A single oversized feature therefore stops the fit rather than quietly moving it to the CPU plane, which is the same standard the [benchmark protocol](../results/benchmark-protocol.md) holds the reference libraries to: a run that asks for a device and trains somewhere else reads as a working fit and is not one.
