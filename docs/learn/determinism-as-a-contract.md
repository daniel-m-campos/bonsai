...
The rule is about CPU training, and the boundary is worth stating plainly rather than leaving to inference. GPU training accumulates histogram cells with device atomics whose landing order is the scheduler's, and for its first year the same fit wrote different model bytes on every run: four repetitions of one `cuda_depthwise` fit at a single commit produced four different model hashes. The cells are now int64 fixed point, so the sum is the same integer in any order and a device fit is bit-identical to itself on one device and one build; the suite fits three times and compares bytes. What the device plane still does not promise is the rest of the CPU rule: identity across devices and toolkits, where the gradient kernels' transcendentals and fused multiply-adds differ, and identity with the host fit, which accumulates float cells and agrees to 1e-4. A narrower claim honestly made rather than the same claim quietly extended.
