...
- **G1. [The marginal round](1-the-marginal-round.md).** Instrumentation cancels a kernel rewrite, then a price list cuts the 16M round from 155 to 104 ms.
- **G2. [The missing bin](2-the-missing-bin.md).** An acceptance test fails, and the fix closes a train/predict skew hiding in every fitted model.
- **G3. [The parity verdict](3-the-parity-verdict.md).** A data-parallel multi-GPU engine, built and measured to parity, then parked as an experiment.
- **G4. [The resident objective](4-the-resident-objective.md).** Deleting the per-tree host round-trip, and the round falls again from 104 to 64 ms.
- **G5. [The ceiling](5-the-ceiling.md).** 500M rows by 100 features trained end to end on one 80GB card.
- **G6. [The wide-data wall](6-the-wide-data-wall.md).** A production field report at 16k features, the cache arithmetic behind a 2-6x cliff, two wrong theories killed by an interleaved A/B, and the tiled layout that dissolves the trade-off.
