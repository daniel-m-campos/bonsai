...
Shared-memory accumulation is float (native atomics; double atomics CAS-loop), cross-chunk and subtraction arithmetic is double; rounding stays bounded per ≤32k-row chunk. The `cuda_*` contract is **tolerance-equal, not bit-equal**: atomic order and warp-tiled prefix association differ from the serial CPU scan, so equal-within-rounding gains may pick different splits (structurally different, equally good trees). Not bit-equal to itself either, which is the part easy to miss: atomic order varies between two runs of the *same* device fit at the same commit, so device model bytes are not reproducible run to run (four repetitions of one `cuda_depthwise` fit at one commit, four different model hashes) and any device-side equivalence claim is argued against that measured spread. Parity tests assert prediction/RMSE tolerance, never tree equality; CPU-only builds remain bit-identical and untouched. Small nodes (<512 rows) use the direct-global kernel because the shared-memory kernel's fixed per-(node,feature) cost is bin-proportional; there is no CPU fallback on the resident path: `begin_root` declines wholesale (oversized `max_bin`) and the host `LevelStep` plane takes over.
...
bonsai wins each structure-matched row. The leafwise row was measured before a device leafwise existed: best-first growth expands one node at a time, which the level-batched resident plane cannot serve (decision 42: a working `cuda_leafwise` was built, benchmarked, and withdrawn for computing CPU histograms under a GPU name), and CPU leafwise beat LightGBM's CUDA backend on this card at this dataset's 464k rows - a small-n reading the 2026-08 recheck inverts at ladder scales (5.3x in LightGBM's favor at 16M; decision 95). The slot-pool leaf plane of [`20-cuda-leafwise.md`](20-cuda-leafwise.md) is the non-swapping advance that answers it; its stage 1 is built and registered, and issue #268's ladder decides whether the registration stays. LightGBM's PyPI wheel is CPU-only: its GPU number is a CUDA source build (`pip install lightgbm --no-binary lightgbm --config-settings=cmake.define.USE_CUDA=ON`); CatBoost caps GPU `border_count` at 254 vs 255 bins elsewhere.
