...
The `populate` → `populate_many` → resident progression was a development ladder: its stages are the phase-1/2/3 story preserved in git history and in doc 11's measured ladder. Only the last step is worth keeping. So this design retires the intermediate machinery: `populate_many`, `BatchHistogramBuilder`, and the GPU **copy-back** histogram path (build on the device, copy home to `SplitInput.hists`). When the resident path declines (a feature's bins overflow the shared-memory budget, or the level buffers would not fit), the CUDA engine falls back to **CPU** histogram construction, delegating through the `cpu` member it already owns for sub-cutoff nodes. The default `max_bin = 255` always goes resident; the fallback is a rare path, and building it on the CPU (rather than a bespoke GPU-copy-back path) trades a slice of fallback throughput for a much smaller CUDA backend.
...
Device oblivious **landed** (decision 42): `find_level_split` is the device level-find this paragraph anticipated, `cuda_oblivious` is registered, and its parity tests ride the cuda suite: see [`11-gpu-resident.md`](11-gpu-resident.md). Device leafwise **landed as stage 1** ([`20-cuda-leafwise.md`](20-cuda-leafwise.md), issue #268), superseding this doc's earlier reading that its sequential gain heap left no device value: the sequential part is the *decision* stream, which stays on the host, and a per-tree histogram slot pool serves a frontier of one where the ping-pong could not. The seam is a `LeafStep` shaped exactly like the `LevelStep` above, host primary template and GPU specialization holding the one runtime fork; the grow loop keeps every decision. The first `cuda_leafwise`, built and withdrawn for running CPU histograms under a GPU name (decision 42), is what that registration must not repeat. One correction to this doc's earlier draft: the oblivious grower's *unsampled-row routing* does not unify with `route_unsampled` (it routes to ObliviousTree leaf-table indices, not DenseTree node ids); only its level update went through the `LevelStep`. The device kernels and their optimization stages are unchanged and remain in [`11-gpu-resident.md`](11-gpu-resident.md); the engine policy originates as the histogram builder in [`10-cuda.md`](10-cuda.md); the ratifying choice is decision 41, superseding the framing of decision 40.
