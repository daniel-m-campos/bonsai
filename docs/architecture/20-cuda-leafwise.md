...
**Adopt: adaptive accumulator width, eventually.** LightGBM's quantized-gradient path picks 8/16/32-bit histogram accumulators from leaf row count, halving histogram traffic exactly where round counts are highest. It is off by default there and out of scope for rung 1 here, but it is the natural follow-up lever if the histogram build dominates after admission.
...
Out of scope for rung 1, wired later: the device-resident objective (`LeafwiseGrower::resident_begin` is a static false today; the resident plane's per-fit machinery is orthogonal and joins after admission), and multi-GPU (doc 19 is parked; nothing here forecloses it).
