...
When the booster arms the device-resident objective (MSE, LogLoss, or Poisson, weighted or not, with no DART and a sampler that never reads gradient values), the boundary crossings above disappear for the whole fit: labels and initial scores upload once, `begin_tree` derives the packed `(grad, hess)` array on the device from the resident scores, and the finalize step replaces the stamp-and-copy epilogue with one kernel that routes every row through the finished tree in bin space and adds its leaf value into the resident scores. Per tree, nothing crosses the bus; the host scores are synchronized once when the fit leaves resident mode. `BONSAI_HOST_OBJECTIVE=1` forces the host path.

## Device predict

Prediction over a Dataset the caller binned on a device reuses the plane the ingest transaction left there. [`include/bonsai/cuda/predict.hpp`](../../include/bonsai/cuda/predict.hpp) declares two calls. `cuda_predict_plan` packs the ensemble once: it inverts every split threshold to its bin through the mappers that cut them, writes the nodes as SoA arrays concatenated over the trees with each tree's base recorded, and uploads them with the per-feature last-bin table. `cuda_predict` launches one thread per row; the thread walks every tree from its base, accumulates the leaf values in a register, and writes `init + lr * sum`. Rows are disjoint, so the walk uses no atomics, and only the per-row scores cross the bus.

The routing rule is `routes_left` from `dataset.hpp`, the same rule the resident epilogue's `route_add_kernel` mirrors. The score epilogue spells its rounding out with `__fadd_rn` and `__fmul_rn`: this translation unit compiles with `-ffp-contract=fast` and the host plane with `off`, so the plain expression would fuse and land one ulp from the host walk's answer, and prediction owes bit equality with that walk. The `[cuda][predict]` case pins it row by row on a device.

Both calls decline rather than fail. There is no plan without a CUDA build, a usable device, or an ensemble the packers can take, so multiclass models ride the host bin walk (a levelwise model does not: it lends the dense equivalent its plan input owns, decision 111); there is no walk over a plane another backend minted or one whose shape disagrees with the plan's. The Python `Model` caches the plan against the booster's mutation epoch and falls through to `predict_at_binned` whenever either call declines. Only the width-1 `predict` over a Dataset rides this route; `predict_proba`, `staged_predict`, `predict_leaf`, and `pred_contribs` stay on the host.
