...
`k_min_gpu_rows = 512` routes each resident child to the right kernel (below it, the direct-global small-node kernel beats the shared-memory one: the fixed zero+merge cost is bin-proportional, small nodes are row-proportional). `BONSAI_HIST_SMALL_THRESH` overrides that cutoff, read once per device context, so the crossover can be swept without a rebuild; the profile line names the live value and counts the nodes that took the small-node kernel. The 48 KiB/block shared-memory budget (`max_bin` ≳ 6k) is checked once per tree in `begin_root`, which declines the resident path and lets the `LevelStep` fall back to CPU histogram building. The launch never fails at runtime.
