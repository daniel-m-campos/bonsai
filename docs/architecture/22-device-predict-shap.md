...
> **Status:** landed for every width-1 model, dense and levelwise (decision 111). Correctness is device-verified on a Jetson Orin (sm_87) for both arms: predict bit-equal to the host walk, SHAP worst element gap 1.17e-7 and worst additivity residual 3.22e-7 against the fp64 reference, over five fixtures. Levelwise throughput is measured on an L40S (decision 111): 22x to 83x over the host walk it replaces, rising with depth, and faster than depthwise on the device from depth 6 up. The cross-library ledger still waits for the standings pod.
...
A levelwise model reaches both planes through that owner (decision 111). Its trees are oblivious, so it attaches the epoch-cached dense equivalent the host SHAP path already builds and points the span at that. Nothing else moves: the packers take `DenseTree` either way, and they copy and upload at pack time, so the owner has only to outlive the pack call. Densification mints a perfect tree, so a depth-d levelwise tree packs 2^d paths whatever its live coverage, and the slots with no evidence behind them pack verbatim carrying a zero cover fraction. Those slots are unreachable rather than merely unvisited: an oblivious tree can split one feature at two levels, so the expansion holds corners that assert `f <= t_i` and `f > t_j` at once. That same repetition is why the path count does not cost what it looks like it should. A path merges one element per distinct feature, so a levelwise path is short where a depthwise path can carry one element per level, and merged length is what picks the kernel template and sets the polynomial degree. Levelwise packs more paths and pays less for each, which measures out as the faster arm from depth 6 up (decision 111). What the kernel meets is a zero-cover element in a path the row does not follow, where the host form divides by that fraction and guards the branch off while the closed form only multiplies and needs none. The plan input is also no longer free to ask for on this arm, so the callers in `module.cpp` ask for it after the gates that do not need it.
