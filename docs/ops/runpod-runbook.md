...
Under a **CFS bandwidth quota** the threads share one pool of core-seconds. A spinning thread is still a running thread, so it draws on that shared pool while it waits, and at threads == quota the fit sits on the ceiling with nothing left for the allocator, the runtime's own threads, or the reference library's helpers. Such a host needs 1.5 cores per thread before its numbers mean anything, which is why a GPU pod at 13.6 cores cannot time a 16-thread axis no matter how many vCPUs `nproc` claims.

**The sizing rule: rent one vCPU per thread, so 16 vCPU at the published 16 threads.** `standings_refresh.py` refuses a `--cpu-vcpu` below the thread count before it rents anything. `standings_refresh_pod.sh` then asserts the container it actually landed in: either no bandwidth quota is present and the cpuset holds at least as many CPUs as threads, or a quota is present and it clears `threads * 1.5`. Anything else fails the axis loudly rather than publishing a number made under a ceiling nobody declared.
