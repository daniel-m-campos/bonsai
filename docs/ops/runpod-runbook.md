...
**Phase 1: measure.** Rents one RTX PRO 6000 Blackwell (96GB, the host the redesigned matrix is measured on), runs `scripts/standings_refresh_pod.sh` detached on the pod, polls the run log, and pulls the axis jsonl files (plus the plane's `ab-gpu.jsonl` or `ab-cpu.jsonl`; the anchor wheel is always fitted, the previous release when `--prev-version` is set) incrementally, every poll rather than only at the end, so a pod that dies late still leaves finished axes on this machine.
...
The pod proves which commit it is measuring before it builds: it fetches every branch and tag, checks the requested commit out, confirms `HEAD` is that commit, and prints `MEASURING <sha>`. Anything else ends the session with a `STANDINGS_REFRESH_FAIL checkout:` line and `failures=1` instead of measuring whatever the clone happened to be holding. The 1.14.0 refresh is why that assertion exists: `git fetch origin <sha>` asks GitHub for a bare object id, which it refuses to serve unless it is a ref tip, and a failed non-final command in an `&&` list is exempt from `set -e`, so the sweep silently measured `main` and still reported `failures=0`. `measure` sends the local `HEAD`, so measuring an unpushed commit now fails at the pod rather than quietly measuring `main`: push the branch first.
