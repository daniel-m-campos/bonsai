...
- **Seven growers, one engine.** `depthwise` (XGBoost-style), `leafwise` (LightGBM-style), `oblivious` (CatBoost-style), their CUDA twins `cuda_depthwise` / `cuda_oblivious`, and the data-parallel multi-GPU pair `cuda_multi_depthwise` / `cuda_multi_oblivious`; with 7 objectives and 3 samplers the dispatch space is 147 statically-typed combinations, selectable per run from config.
