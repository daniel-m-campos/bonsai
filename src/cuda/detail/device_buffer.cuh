...
// BONSAI_HIST_TILE=W builds a tile-blocked duplicate of the bin plane and
// points the depthwise histogram kernel at it (research probe; 0 or absent
// keeps the feature-major plane alone). Read once per device context, not
// once per launch, so a process can change it between fits without paying
// for the lookup on the hot path.
inline uint32_t hist_tile_env()
{
    char const *set = std::getenv("BONSAI_HIST_TILE");
