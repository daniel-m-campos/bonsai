...
- **Host RAM is the cgroup limit, not what `free` shows**: read `/sys/fs/cgroup/memory.max` (BLACKWELL_96 pods cap at 188GB against 1.5TB physical). Size host-side data generation to ~60% of the cap; `killed by signal 9` on the worker is the symptom of ignoring this.
...
- r² must EQUAL the before-run exactly for behavior-preserving CPU changes; GPU-vs-GPU comparisons are tolerance-equal at 1e-4 (atomic order), and a single-rep flutter in the 4th decimal is issue #278's known phenomenon: check the sibling reps before blaming the change.
