...
4. PR #502, 2026-09-22. Found: `vocabulary_singletons` +4 on the first draft of the host plane choice (`budget`, `binds`, `hands`, `wins`, then `fits` and `max` on the second), each replaced by a word the tree already used (`leaves_bounded`, `on_level_plane`, `node_arena_bytes`), so the row read -1 with `bounded` retired and nothing admitted; `contract_prose_lines` +8 pinned as the three predicates' contracts. The read found what the numbers could not: `node_arena_bytes` re-derived `ArenaLayout`'s packing rule in one line per branch, and now asks the layout. No false positive among the gated rows.
