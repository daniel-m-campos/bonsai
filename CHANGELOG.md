...
### Fixed
- **The auto thread count now clamps to the cgroup CPU quota** (issue #359). `n_threads = 0` resolved OpenMP's max-thread count capped at 16, and that count follows the cpuset affinity mask, which a Kubernetes CPU limit or `docker --cpus` does not narrow: in a quota-limited container bonsai sized its pool from the cores it was shown rather than the CPU it was allowed to burn, drew at the quota rate, and spent the rest of every period frozen by the scheduler. On a container advertising 128 CPUs with a 13.6-CPU quota, 16 threads left 97% of periods throttled where XGBoost left 1.2%. Auto now reads `cpu.max` (cgroup v2) or `cpu.cfs_quota_us` over `cpu.cfs_period_us` (v1) and takes the floor of the ratio as a third bound alongside the 16-thread cap; the read is Linux-only and a host with no quota resolves exactly as before, so nothing changes off a container. Explicit counts still pass through uncapped, and because the fixed-N contract keys model bits to the resolved count, an auto fit on a quota-limited container will produce different model bytes than it did before the clamp. That contract is also why an explicit count is not clamped: silently lowering it would make one configuration produce different models in different containers, so an explicit `n_threads` above the quota instead prints one warning to stderr, once per process, naming the requested count, the detected quota, and the two ways out (lower the count, or set `OMP_WAIT_POLICY=passive`). XGBoost clamps explicit counts as well, making no equivalent reproducibility promise. `OMP_WAIT_POLICY=passive` remains the knob for oversubscribed deployments: on that container it removed 22% of the CPU-seconds, 86% of the throttled time, and 5 to 11% of train.

### Removed
- **`bonsai.bench.airline` is gone, with the airline benchmark axis it served.** The suite is retired rather than deprecated: the axis is not part of the redesigned standings, nothing else imported the module, and `bonsai.encoding` (the ordered target encoder the `bonsai_ts_*` arms exercised) is a product feature and stays. The `airline` entry is out of the dataset registry too, so `python -m bonsai.bench.datasets --list` no longer offers CSVs nothing fetches.
- **The retired bundled specs.** `standings-rows`, `standings-cols`, `standings-cols-full`, `iso-volume-2026-08`, `iso-volume-33-2026-08`, `gpu-pareto-16M`, and `early-stop-4M` are replaced by the redesigned scenario matrix: `gpu-tall`, `gpu-wide`, `gpu-extreme`, `cpu-tall`, `cpu-wide`, and `gpu-early-stop`. `python -m bonsai.bench specs` lists what a wheel actually carries, and `--spec` still takes a path to your own JSON.

### Changed
- **The CPU sparse-node histogram fill is one decomposition, buffer and reduce** (decision 105, issue #360). A level's sparse nodes are cut into fixed 1024-row blocks, the node-major block list is split into one contiguous range per thread, the lowest range touching a node accumulates straight into it, and every higher range owns one partial summed afterwards in range order. This replaces the per-node block plan and its per-block partial slab, and it bounds a level's partial buffers at `n_threads - 1` however many nodes the level holds, where the old plan grew them with the node count: peak RSS falls, and train falls 3% on a 12-thread EPYC, 0 to 2% on a 16-thread Xeon, and not measurably on an M2. `BONSAI_HIST_REDUCE`, which selected between the two while they were being compared, is gone. **Depthwise and levelwise CPU model bytes change at a fixed thread count**, because a node's summation order now depends on the level-wide block partition; predictions move within float noise and accuracy is unchanged, but a saved model refit under this version will not hash to one fit before it. Fits remain bit-identical run to run at a fixed configured thread count, the contract [`docs/architecture/7-parallel.md`](docs/architecture/7-parallel.md) states, and the `cuda_*` growers and the u16 and dense-node fills are untouched.
- **The bench VRAM sampler prefers NVML.** `nvidia-ml-py` joins the `bench` extra, and `DeviceMemSampler` reads per-process device memory through NVML when it is available, falling back to shelling `nvidia-smi` when it is not. The `source` field on every `dev_mem` block says which one produced the numbers, because only NVML attributes memory to the child process inside a container: `nvidia-smi --query-compute-apps` reports host-namespace pids there, which is why per-process VRAM came back null on the pods that measured the old standings.
