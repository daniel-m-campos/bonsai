...
### Added
- **Multi-GPU growers** (architecture doc 19, P4a): `cuda_multi_depthwise` and `cuda_multi_oblivious` train data-parallel across a device set given by `parallel.device_ids` (a TOML integer array or `--set parallel.device_ids=0,1`; empty means the current device). Rows shard across devices, each builds its full histograms per level, and the coordinator reduces them before finding the split. Setting both `parallel.device_id` and `parallel.device_ids` is ambiguous and raises `ConfigError`. Placement only, not persisted in the model artifact. Bring-up status: the full grower matrix and per-device gradient path are wired and parity-tested against the single-GPU engine; sharded ingest is deferred.
