...
### Added
- **`booster.eval_interval`, evaluating the valid set every k-th round** (issue #326): the per-round eval predicts the new tree over the valid rows on the host and scores it, which the GPU growers pay as 7 to 10 percent of fit time where the reference libraries pay 1 to 3. Setting the interval to k pays it once per k rounds plus the final round (CatBoost's `metric_period`); the skipped rounds' trees fold into the running valid scores in one batched pass at the next evaluated round, in round order, so the reported losses are bit-identical to every-round evaluation. Early stopping still counts patience in rounds but only checks it where a loss exists, so a stop can overshoot the every-round stop by up to k - 1 rounds; `best_iteration` is the best evaluated round and truncation follows it, unchanged from interval 1. `evals_result()` returns one entry per evaluated round. Default 1, byte-identical to the previous behavior, and the knob is fit-time only: it is not written into saved models.
...
- **`fit(..., verbose=...)`** is gone. It was accepted and ignored. bonsai prints one line on an early stop and nothing per round.
