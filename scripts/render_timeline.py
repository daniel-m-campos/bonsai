...
def schema_errors(entries: list[dict]) -> list[str]:
    ...
    for i, e in enumerate(entries):
        ...
        if e["bonsai_status"] not in STATUSES:
            problems.append(f"{where}: bonsai_status {e['bonsai_status']!r} not in {STATUSES}")
...
def link_errors(entries: list[dict]) -> list[str]:
    ...
    for e in entries:
        ...
        if not target.is_file():
            problems.append(f"{e.get('title', '?')}: bonsai_link file not found: {path_part}")
            continue
...
def render() -> str:
    ...
    rows = [
        [
            str(e["year"]),
            idea_cell(e),
            carried_by(e["adopters"]),
            f"[{e['bonsai_status']}]({e['bonsai_link']})",
        ]
        for e in entries
    ]
    table = md_table(["Year", "Idea", "Carried by", "bonsai"], rows)
    legend = md_table(["Status", "Meaning"], [[f"`{s}`", STATUS_LEGEND[s]] for s in STATUSES])
    parts = [
        HEADER,
