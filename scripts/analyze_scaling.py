...
curves, and writes benchmarks/scaling-report.md plus PNGs under
benchmarks/results/scaling/. Wired as `make scaling-report`; the report is
committed, so re-run after appending to scaling.jsonl.
...
def main() -> int:
    ...
    ap.add_argument("--out", default=str(REPO / "benchmarks/scaling-report.md"))
    args = ap.parse_args()
    ...
    lines = ["# Scaling suite results", "",
             "Generated by scripts/analyze_scaling.py from benchmarks/results/scaling.jsonl. "
             "Exponent a means fit_s ~ axis^a (log-log slope); cols uses a "
             "joint 2D fit over the rows+cols cells because rows shrink past "
             "4k cols.", "",
             f"Data window: {len(raw)} rows, {ts.min()} to {ts.max()}, "
             f"{raw.git_sha.nunique()} code versions across "
             f"{raw.host_name.nunique()} hosts. This file is the append-only "
             "history: rows keep the code version they measured, so exponents "
             "here can predate later engine work. Current standings live in "
             "the results ledger (docs/method/results.md); the wide-GPU cells "
             "below predate the 2026-07-30 re-baselines (decisions 90 and "
             "91).", ""]

    for host in sorted(raw.host_name.unique()):
        h = raw[raw.host_name == host].iloc[-1]
        host_ts = raw[raw.host_name == host].ts.str[:10]
        lines += [f"## {host}", "",
                  ...
                  + f" · runs {host_ts.min()} to {host_ts.max()}", ""]
