...
# ///
"""Scaling benchmark suite: synthetic regression swept over rows / cols / bins
/ threads, bonsai vs xgboost / lightgbm / catboost (see benchmarks/README.md).
...
def detect_host(name: str | None) -> dict:
    gpu, vram = None, None
    try:
...
def main() -> int:
    ...
    host = detect_host(args.host_name)
    ...
    def emit(cell, variant, threads, repeat, payload):
        line = {"ts": datetime.datetime.now(datetime.UTC)
                .isoformat(timespec="seconds"),
                "git_sha": git_sha, "host": host, "cell": cell,
