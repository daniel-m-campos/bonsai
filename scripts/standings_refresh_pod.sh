...
#   AXES= GIT_SHA=<sha> PLANE=cpu PREV_VERSION=2.2.0 ANCHOR_VERSION=1.15.0 bash scripts/standings_refresh_pod.sh
...
# AXES may be empty on the cpu plane only: that session rents a cpuset host
# for the cpu A/B alone while the cpu axes ride the gpu pod (see run_cpu_ab).
AXES="${AXES:-}"
...
PLANE="${PLANE:-gpu}"
if [ -z "$AXES" ] && [ "$PLANE" != cpu ]; then
    echo "AXES: comma list of axes; empty only on the cpu plane" >&2
    exit 1
fi
...
# The cpu A/B runs only on the cpu plane's pod, even when the cpu axes ride
# the gpu pod. A ranking needs the gpu pod's server silicon (a cpu rental's
# desktop-class part moved lightgbm 3.0x and flipped the order), but a
# same-library A/B needs a quiet host: the gpu pod's bandwidth-metered cpu
# spread 15-22% run to run at the A/B cell, which no min over reps resolves
# against a 2% band, and a 4-rep min there read a +14% move that 18
# interleaved reps put at -2%. A cpuset host spread 0.7% at the same cell.
run_cpu_ab() {  # <threads>
    [ "$PLANE" = cpu ] || return 0
...
# The A/B without its axis: the same cap gate as run_cpu_axis, with the
# throttle counters bracketing the whole A/B since there is no probe fit.
run_cpu_ab_alone() {
    threads=$(spec_field "$CPU_AB_AXIS" "max(s.get('threads', [16]))")
    ab=/root/standings/ab-cpu.jsonl
    cpu_cap_ok ab-cpu "$threads" "$ab" || return 0
    read -r p0 t0 <<< "$(throttle_counters)"
    run_cpu_ab "$threads"
    read -r p1 t1 <<< "$(throttle_counters)"
    pct=$(throttled_pct "$((p1 - p0))" "$((t1 - t0))")
    echo "QUOTA ab-cpu: throttled ${pct}% of $((p1 - p0)) enforcement periods around the A/B"
    throttle_ok ab-cpu "$pct" "$threads" "$ab"
}
...
fail_axis() {  # <axis> <why> [partial file, default the axis's dated rows]
    echo "STANDINGS_REFRESH_FAIL $1: $2"
    echo "$1: $2" >> "$QUOTA_FAIL"
    partial="${3:-/root/standings/$1-$YM.jsonl}"
    [ -f "$partial" ] && mv "$partial" "$(dirname "$partial")/QUOTAFAIL-$(basename "$partial")"
    FAILURES=$((FAILURES + 1))
}

# The static half of the CPU-cap gate: nonzero, and the axis failed, when
# the container cannot carry the threads. Which cap the host enforces
# decides what the axis needs, because it decides whether a spinning barrier
# costs anything. A cpuset host hands the container whole CPUs, so one CPU
# per thread is enough and a spinning thread only burns the core it already
# owns. A bandwidth host meters core-seconds across all threads, so spin-wait
# eats the shared allowance and the quota has to exceed the thread count,
# with the measured throttle share (throttle_ok) as the real check.
cpu_cap_ok() {  # <axis> <threads> [partial file]
    quota=$(quota_cores)
    usable=$(nproc)
    echo "CPUCAP $1: quota=${quota}, usable=${usable} cpus, spec=${2}t"
    if [ "$quota" = unlimited ]; then
        if [ "$usable" -lt "$2" ]; then
            fail_axis "$1" "no bandwidth quota, so the cpuset is the cap, and its ${usable} cpus are fewer than the ${2} threads the spec claims; rent a wider cpu pod (--cpu-vcpu) and re-run the axis" "${3:-}"
            return 1
        fi
        return 0
    fi
    required=$(awk -v t="$2" -v s="$QUOTA_SPARE_CORES" \
        'BEGIN {printf "%.2f", t + s}')
    if awk -v q="$quota" -v r="$required" 'BEGIN {exit !(q < r)}'; then
        fail_axis "$1" "this host meters cpu bandwidth at ${quota} cores, below the ${required} its ${2}t claim needs (one spare core, so the fit is not pinned to the ceiling); draw another pod or lower the spec's thread count" "${3:-}"
        return 1
    fi
}
...
throttled_pct() {  # <periods> <throttled>
    awk -v p="$1" -v t="$2" \
        'BEGIN {printf "%.1f", (p > 0 ? 100 * t / p : 0)}'
}

# The measured half of the gate: nonzero, and the axis failed, when the
# throttled share is over the limit or could not be read at all, since a
# gate that cannot read its own counters has measured nothing.
throttle_ok() {  # <axis> <pct> <threads> [partial file]
    local axis=$1 pct=$2 threads=$3 partial=${4:-}
    case "$pct" in
        ''|*[!0-9.]*)
            fail_axis "$axis" "the throttle probe produced no percentage, so nothing was measured to gate on; a gate that cannot read its own counters must not pass the axis" "$partial"
            return 1 ;;
    esac
    if awk -v pct="$pct" -v max="$THROTTLED_PCT_MAX" 'BEGIN {exit !(pct + 0 > max + 0)}'; then
        fail_axis "$axis" "throttled in ${pct}% of CFS periods at ${threads}t (limit ${THROTTLED_PCT_MAX}%); this host cannot time the cpu plane" "$partial"
        return 1
    fi
}
...
run_cpu_axis() {
    ...
    cpu_cap_ok "$axis" "$spec_threads" || return 0
    ...
    throttle_ok "$axis" "$pct" "$spec_threads" || return 0
...
if [ "$PLANE" = cpu ] && [[ ",$AXES," != *",$CPU_AB_AXIS,"* ]]; then
    run_cpu_ab_alone
fi
