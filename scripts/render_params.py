"""Render docs/api/parameters.md from the committed config extraction.
...
type, a default, and an effect on the model.
    ...
    bonsai params | python3 scripts/render_params.py --extract --check  # CI: JSON vs structs
    python3 scripts/render_params.py                                    # rewrite page
    python3 scripts/render_params.py --check                            # CI: page vs JSON
...
SRC = REPO / "docs" / "api" / "parameters.src.json"
OUT = REPO / "docs" / "api" / "parameters.md"
...
EFFECTS = {
    ...
    # tree
    "tree.min_child_hess": "Minimum hessian each child of a split must hold, or the candidate is rejected. Under squared error a row weighs 1, so this counts rows.",
    ...
    # parallel
    "parallel.n_threads": "CPU threads for training. 0 auto-detects hardware threads, capped at 16 and at the CPU quota of a quota-limited container. An explicit count over that quota is honored, so the model stays reproducible, and warns once. Set OMP_WAIT_POLICY=passive where the pool is oversubscribed. Worker threads bind to one core each, one package first, unless OMP_PROC_BIND or OMP_PLACES is set; OMP_PROC_BIND=false leaves placement to the scheduler, for several fits sharing one container.",
...
def extract() -> int:
    """Write the JSON extraction of `bonsai params` on stdin, or `--check` it.

    The check compares the fresh dump against the committed file and names
    every knob that appeared, vanished, or changed its default, so the
    author learns which declaration moved rather than that something did.
    """
    out = extraction(sys.stdin.read())
...
def extraction_drift(committed: dict[str, dict], fresh: dict[str, dict]) -> list[str]:
    """Dotted knobs on which the committed extraction and a fresh one differ."""
    flat_old = {f"{s}.{k}": v for s, keys in committed.items() for k, v in keys.items()}
...
HEADER = """<!-- GENERATED by scripts/render_params.py from docs/api/parameters.src.json. Edit the generator or run `make params-json`, not this page. -->
...
The table below is generated from `docs/api/parameters.src.json`, extracted from the default `Config` by `make params-json` (which runs `bonsai params`). CI checks the page against that JSON and the JSON against the built CLI, so a knob added or renamed in the structs fails the build until `make params-json` is re-run.
"""
