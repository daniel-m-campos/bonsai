...
# ///
"""Compute-DAG placement model (method: docs/results/benchmark-protocol.md).
...
NODES = [
    Node(
        ...
        0.31,
        0.30,
        note="tall 16M x 128 same-pod 2026-09-09: host 0.31-0.33 s, device 0.30 s; both draw the seeded 200000-row sample on the host (about 0.26 s at 16M, model-changing to move), so the device sort pays only where every row is cut",
    ),
    Node(
        "bin",
        4.65,
        0.55,
        note="decision 54 landed: dbin kernel + streamed upload (host cost kept for the counterfactual)",
    ),
    Node(
        "gradients_scores",
        0.51,
        None,
        pinned="host",
        note="objective+sample+score fit-profile lines",
    ),
    Node(
        "hist_build",
        None,
        7.90,
        pinned="device",
        note="adv_hist+root_hist events; the round's dominant cost",
    ),
    Node(
        "root_sums",
        None,
        1.02,
        pinned="device",
        note="decision-72 two-pass reduce, replaces the host loop",
    ),
    ...
    Node(
        ...
        None,
        pinned="host",
        note="doc 12 control plane; per-level sync floor",
    ),
]
...
def evaluate(placement: dict[str, str]) -> tuple[float, list[str]]:
    ...
    for e in EDGES:
        src = (
            placement.get(e.src, by_name[e.src].pinned if e.src in by_name else "host")
            if e.src in by_name
            else "host"
        )
        ...
        if src != dst:
            ...
            total += c
            lines.append(
                f"  {e.src}->{e.dst:18s} xfer  {c:5.2f}s  ({e.bytes_per_fit / GB:.1f}GB {e.bw})"
            )
    return total, lines
