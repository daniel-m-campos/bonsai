...
_STANDINGS_REG = json.loads((REPO / "benchmarks" / "standings.json").read_text())
...
def provenance(files: list[str], note: str) -> str:
    ...
    links = ", ".join(f"[`{f}`](../../benchmarks/results/{f})" for f in files)
...
def bar_chart(fname: str, title: str, rows: list[tuple[str, float, str]], x_max: float, note: str):
    ...
    for i, (label, value, note_txt) in enumerate(rows):
        ...
        body.append(
            f"<rect x='{left}' y='{y}' width='{bw}' height='20' "
            f"rx='3' fill='{color}' fill-opacity='0.85'/>"
        )
        body.append(_text(left + bw + 8, y + 14, f"{value:.2f}&#160;&#160;{note_txt}"))
...
def _standings_tables(
    ...
    chart: str,
    plane: str,
    knobs: str = "campaign knobs",
    arms: str = "every library",
):
    ...
    per_suite = md_table(
        ...
        [
            [
                lib,
                *[
                    fmt(sum(v) / len(v), 2) if (v := suite_ranks.get((s, lib))) else "-"
                    for s in suites
                ],
            ]
            for lib, _, _ in table
        ],
    )
...
def grinsztajn_leaf_capped_section() -> str:
    ...
    standings, per_suite, n = _standings_tables(
        rows,
        "grinsztajn-rank-leaf-capped-gpu.svg",
        "GPU",
        knobs="leaf-capped knobs",
        arms="bonsai leafwise and LightGBM",
    )
...
def grinsztajn_drift_section() -> str:
    ...
    if not gpu:
        return ""
    drifts = check_standings.quality_drift(load_jsonl(standings_file(Axis.GRINSZTAJN)), gpu)
    ...
    table = md_table(
        ...
        [
            [
                d.grower,
                check_standings.QUALITY_PARTNERS[d.grower],
                str(d.pairs),
                f"{d.mean:+.2e}",
                f"{d.worst:.2e}",
                f"{d.worst_spread:.2e}",
...
PLANE_LABEL: Final = {PLANE_GPU: "The GPU plane is", PLANE_CPU: "The CPU plane is"}

PENDING: Final = (
    "*Measurement pending the first redesigned refresh "
    "(decision 103): the axis files land with it and this "
    "section fills in unchanged.*"
)
AB_AXES: Final = (Axis.GPU_TALL, Axis.CPU_TALL)
PENDING_AB: Final = (
    "*Pending the first refresh that fits the anchor arm: "
        ...
        for i, arm in enumerate(panel.labels()):
            ...
            body.append(
                [
                    ...
                    *[
                        _compare(
                            c, cells[i][c], cells[j][c], _tie_floor(spreads[i][c], spreads[j][c])
                        )
                        for c in columns
                    ],
                ]
...
def _ab_block(axis: str) -> str:
    ...
    return (
        f"### {plane} plane ({check_standings.ab_arm_line(rows)})\n\n"
        f"{check_standings.ab_table(rows)}\n\n"
        + provenance(
            [name],
            "Min over the session's interleaved "
        ...
        for variant, arm in zip(panel.arms(PLANE_GPU), panel.labels()):
            ...
            body.append(
                [
                    panel.grower,
                    arm,
                    _overhead(off, evaluated),
                    _fit_text(stopped),
                    fmt(stopped.get("stopped_at") if stopped else None),
                ]
            )
    table = (
        md_table(["grower", "arm", "eval overhead", "stop-arm fit", "stopped at"], body)
        if rows
        else PENDING
    )
...
def shap_section() -> str:
    ...
    arms = (
        ("bonsai_cuda_depthwise", "bonsai depthwise (GPU)"),
    ...
    def stat(variant: str, key: tuple, field: str, pick) -> float | None:
        vals = [
            r.get(field)
            for r in rows
            if r.get(K.VARIANT) == variant
            and cell_key(r) == key
            and r.get(K.STATUS) == "ok"
    ...
    for key in cells:
        ...
        fid_body.append(
            [label]
            + [
                "-" if (a := stat(v, key, K.CONTRIBS_ADDITIVITY, max)) is None else f"{a:.1e}"
                for v, _ in arms
            ]
        )
    headers = ["cell"] + [label for _, label in arms]
    tables = (
        (md_table(headers, time_body) + "\n\n" + md_table(headers, fid_body)) if rows else PENDING
    )
...
def _arm_spreads(
    rows: list[dict], variant: str, columns: tuple[str, ...]
) -> dict[str, float | None]:
    """One arm's observed spread per column, across the scenario's repeats."""
    finished = [
        _arm_cells(r, columns) for r in rows if r[K.VARIANT] == variant and r[K.STATUS] == "ok"
    ]
    return {c: _spread([cells[c][1] for cells in finished]) for c in columns}


def _compare(
    column: str, mine: tuple[str, float | None], theirs: tuple[str, float | None], floor: float
) -> str:
...
def _fit_arms(axis: str, panel: Panel, plane: str) -> tuple[dict | None, dict | None, bool]:
    ...
    spreads = (
        _spread([_ok_value(r, K.FIT_S) for r in rows if r[K.VARIANT] == v])
        for v in panel.arms(plane)
    )
    tied = _is_tie(_ok_value(bonsai, K.FIT_S), _ok_value(rival, K.FIT_S), _tie_floor(*spreads))
...
def _summary_cell(panel: Panel, axis: str, plane: str) -> str:
    ...
    if bonsai is None or rival is None:
        return "-"
    if tied:
        return f"bonsai {_fit_text(bonsai)} vs {panel.rival} {_fit_text(rival)} {TIE_NOTE}"
    ours, theirs = (_ok_value(r, K.FIT_S) for r in (bonsai, rival))
    return (
        f"{_bold('bonsai ' + _fit_text(bonsai), _wins('fit', ours, theirs))}"
        f" vs "
...
def _mode_row(rows: list[dict], variant: str, mode: str) -> dict | None:
    ...
    return best_row(
        [r for r in rows if (r.get("eval_mode") or (r.get("cell") or {}).get("eval_mode")) == mode],
        variant,
    )
...
def _perf_provenance() -> str:
    ...
    stamps = " ".join(
        s
        for s in (measured_stamp(PLANE_LABEL[plane], rows) for plane, rows in _perf_plane_rows())
        if s
    )
    return (
        "\n"
...
def _plane_table(planes: list[dict]) -> str:
    ...
    return md_table(
        ...
        + [
            [
                "all",
                *[str(sum(p[k] for p in planes)) for k in ("files", "loc", "nloc", "functions")],
                "-",
                "-",
            ]
        ],
...
GEN_NOTE = "<!-- GENERATED by scripts/render_results.py. Edit the generator, not this page. -->"
...
PAGES: list[tuple[str, str, str, list]] = [
    (
        "perf.md",
        "The scenario panels",
        "Every grower against its closest rival, both planes, at the redesigned scenarios.",
        [perf_summary_section, perf_panels_section, early_stop_section, shap_section, ab_section],
    ),
    (
        "quality-grinsztajn.md",
        "Grinsztajn standings",
        "The only citable standings: 55 third-party tasks.",
        [
            grinsztajn_section,
            grinsztajn_gpu_section,
            grinsztajn_drift_section,
...
def _perf_summary() -> str:
    ...
    if not gpu and not cpu:
        return (
            "The perf standings are being re-measured on the redesigned "
    ...
    parts = []
    if gpu:
        parts.append("On GPU at the tall scenario, fit totals run " + "; ".join(gpu) + ".")
    if cpu:
        parts.append("On CPU at the tall scenario: " + "; ".join(cpu) + ".")
    parts.append(
        "The wide and extreme scenarios, the host and device memory "
...
def _headline_clauses(axis: str, plane: str) -> list[str]:
    ...
    for panel in PANELS:
        ...
        out.append(f"{panel.grower} {ours:.1f}s vs {panel.rival} {theirs:.1f}s{note}")
    return out
...
def render_pages() -> dict[pathlib.Path, str]:
    ...
    for rel, title, _desc, fns in PAGES:
        ...
        out[REPO / "docs" / "results" / "results" / rel] = f"{GEN_NOTE}\n\n{head}{body}"
...
def check_registry(consumed_files: set[str]) -> list[str]:
    ...
    for axis, e in reg.items():
        ...
        errors += _registered_file_findings(axis, "companion ", e.get("companion"), consumed_files)
        errors += _registered_file_findings(axis, "ab ", e.get("ab"), consumed_files)
        if (RESULTS / e["file"]).exists() and e.get("sha"):
            errors += _row_sha_findings(axis, e, RESULTS / e["file"])
    return errors


def _registered_file_findings(
    axis: str, kind: str, name: str | None, consumed_files: set[str]
) -> list[str]:
...
def _row_sha_findings(axis: str, entry: dict, path: pathlib.Path) -> list[str]:
    ...
    sha = entry["sha"]
    wrong = {s for s in shas if s and not s.startswith(sha) and not sha.startswith(s)}
    errors = []
    if wrong:
        errors.append(f"standings {axis}: rows carry {sorted(wrong)}, registry says {sha}")
    if None in shas and not entry.get("sha_partial"):
        errors.append(f"standings {axis}: rows without git_sha in a full-provenance standings file")
    return errors
...
def main() -> int:
    ...
    if missed:
        print(
            f"ERROR: committed results files not rendered: {sorted(missed)}\n"
    ...
    orphans = (
        [p for p in pages_dir.glob("*.md") if p not in outputs and p.name not in HAND_WRITTEN_PAGES]
        if pages_dir.is_dir()
        else []
    )
    if orphans:
        ...
        print(
            f"ERROR: orphaned generated pages: {names}; remove them or wire them into PAGES.",
            file=sys.stderr,
        )
        return 1
    return write_or_check(
        ...
        detail=f"{len(PAGES)} pages + landing, {len(charts)} charts",
    )
