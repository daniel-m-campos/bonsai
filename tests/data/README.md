...
|---|---|---|---|
| test-pin (CI-load-bearing) | `tiny.csv` (committed), `california_housing_*.csv`, `amazon_*.csv` | `uv run scripts/fetch.py california` and `amazon` (the registry; CI cache keys hash the registry and the script) | C++ eval-baseline pin, Python binding tests, encoder quality pins. Do not move or rename. |
| quality-external | `a9a_*.libsvm`, `covtype_*.csv` | `uv run scripts/fetch.py <name>` | benchmark-only, gitignored |
